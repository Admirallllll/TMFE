...
def fit_lasso_ngram(
    ...
    texts = merged[text_col].fillna("").tolist()
    ...
    # Scale target (Lasso is scale-sensitive for the output)
    # Features already normalized by TF-IDF; target we keep in original scale
    # Use pre-scaled X with StandardScaler
    ...
    y_pred = cross_val_predict(
        LassoCV(alphas=[alpha], cv=cv, max_iter=2000, n_jobs=1), X_scaled, y, cv=cv
    )
    ...
    print(f"  Kendall's Tau:  {tau:.4f}  (p={p_val:.4f})")
...
def run_lasso_text_analysis(
    ...
    if initiation_scores_path and os.path.exists(initiation_scores_path):
        ...
        if "ai_initiation_score" in init_df.columns and len(init_df) > 30:
            targets.append(("ai_initiation_score", corpus_df, init_df))
        else:
            print("Skipping ai_initiation_score: not enough data.")

    all_results = {}

    for target_col, corp_df, target_df in targets:
        print(f"\n{'='*60}")
