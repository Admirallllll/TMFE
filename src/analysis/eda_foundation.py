...
DEFAULT_REPORT_DIR = os.path.join("outputs", "reports", "eda")
...
def _save_funnel_reports(funnel: Dict[str, float], report_dir: str) -> None:
    os.makedirs(report_dir, exist_ok=True)
    pd.DataFrame([funnel]).to_csv(os.path.join(report_dir, "funnel_summary.csv"), index=False)
    _save_json(os.path.join(report_dir, "funnel_summary.json"), funnel)
    ...
    with open(os.path.join(report_dir, "funnel_summary.txt"), "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
...
def run_eda_foundation(
    ...
    _save_funnel_reports(funnel, report_dir)
