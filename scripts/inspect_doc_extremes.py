...
def extract_context(sentences_df, doc_id, section, filter_col, top_n=3, window=2):
    ...
    doc_df = doc_df.sort_values(['turn_idx', 'sentence_idx']).reset_index(drop=True)
...
def inspect_documents():
    ...
    sentences_df = pd.read_parquet("outputs/features/sentences_with_keywords.parquet")
    ...
    for category, doc_id, target_sections in targets:
        ...
        for tgt_sec in target_sections:
            ...
            print(extract_context(sentences_df, doc_id, sec, 'kw_match_count', top_n=5, window=2))
